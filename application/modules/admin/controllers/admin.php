...
class admin extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->load->model(array('admin_model','report_model'));
    ...
    function ajax_get_not_approved_user() {
        $this->datatables->select('ut.id, ut.kode_agent, at.name as agent_name, ut.mg_user_id, ut.email as email_user,user_date_create,sales_rekomend',FALSE)
    ...
    function ajax_get_approved_user() {
        $this->datatables->select('ut.id, ut.name as name_user,ut.address,ut.city as kota,ut.kode_agent,at.name as agent_name,mg_user_id,ut.email,point,user_date_create,user_date_approved,sales_rekomend, ut.status',FALSE)
        ->edit_column('user_date_approved', '$1', 'format_date(user_date_approved)')
        ->edit_column('user_date_create', '$1', 'format_date(user_date_create)')
        ->edit_column('ut.status', '$1', 'status_user(ut.status)')
        ->add_column('Actions', '$1', 'Actions_User(ut.id, ut.status)')	
        ->unset_column('ut.id')
        ->unset_column('ut.address')
        ->from('user_table AS ut')
        ->join('agent_table  AS at','ut.kode_agent = at.kode_agent')
        ->where('ut.status !=', '0');
        echo $this->datatables->generate();
    ...
    function ChangePassword(){
        $data_update = array(
            'password'  => encryptPass($this->input->post('ChangePassword'))
        );
        $key        = array(
            'id'    => $this->input->post('idUser')
        );
        $ChangePassword     = $this->general_model->updateData('user_table',$data_update,$key);
        if($ChangePassword){
            $alert = 'Password success to change';		
            $returnVal = 'success';
        }else{
    ...
    function aprrove_register(){
        ...
        $EmailRekomend    = $this->input->post('u_EmailRekomend');
        ...
        if($update_user){
            if(!empty($EmailRekomend)){
                if(strpos($EmailRekomend,'@mgholiday.com') === false){
                    $pointRekomend      = 5 ;
                    $checkUSerAktif     = $this->general_model->getDetData('user_table',array('email' => $EmailRekomend, 'status' => 1));
                    $checkUSeremail     = $this->general_model->getDetData('user_table',$fieldkey);
                    if($checkUSerAktif && ($checkUSerAktif->email != $checkUSeremail->email)){
                            $insertHistory  = array(
                            'user_id'       => $checkUSerAktif->id,
                            'status'        => 'income',
                            'last_point'    => $checkUSerAktif->point,
                            'in_point'      => $pointRekomend,
                            'current_point' => ($checkUSerAktif->point + $pointRekomend),
                            'date_create'   => date('Y-m-d H:i:s'),
                            'id_admin_approve' => $CMS_Session['id'],
                            'notes'         => 'By Register Rekomendasi'
                        );
                        $History        = $this->general_model->insertData('history_user',$insertHistory);
                        $update_user    = $this->general_model->updateData('user_table',array('point' => ($checkUSerAktif->point + $pointRekomend)), array('id' => $checkUSerAktif->id));                        
                    }
                }                
            }
    ...
    function ajax_get_gift_request() {
        ...
        $this->datatables->select('hu.id as id_req_Gift, ut.kode_agent, at.name as agent_name, DATE_FORMAT(hu.date_create, "%d %M %Y") as date_claim, ut.mg_user_id, ut.name as username, ut.nama_bank, ut.no_rekening, ut.atas_nama_bank, gt.name as giftname, gt.value, hu.status as status_req',FALSE)
    ...
    function approval_gift_request_list() {
        //$this->_check_login();
        $this->load->module('admin/report');
        $session_data = $this->session->userdata('CMS_logged_in');
        $data['admin'] = $session_data['username'];
        $this->form_validation->set_rules('date_search', 'Date Claimed', 'required|callback_check_claimed');
        if ($this->form_validation->run($this) == TRUE) {
            $this->report->ClaimedAprroved($this->input->post('date_search'));
        } else {
    ...
    function check_claimed($str) {
        $this->form_validation->set_message('check_claimed', 'No records.');
        if ($this->report_model->report_claimed($str)) {
            return TRUE;
        } else {
            return FALSE;
        }
    ...
    function getDetailReqGift(){
        ...
        $data['nama_bank']      = $get->nama_bank;
        $data['no_rekening']    = $get->no_rekening;
        $data['atas_nama_bank'] = $get->atas_nama_bank;
    ...
    function approve_gift_request() {
        ...
        if($updateReqGift){
            $getDetUser             = $this->admin_model->getDetData_User($idUser)  ;
            ...
            switch($getDetGift->type){
                case '1' :
                    $content_message .= '<p><h4>Hi, '.$getDetUser->mg_user_id.'/'.$getDetUser->kode_agent.' - '.$getDetUser->agent_name.'</h4></p>
	...
	if(empty($alert)){
		...
		if($result){
                    foreach($result as $row){
                        ...
                        $details['nama_bank']	        = $row['nama_bank'];
                        $details['no_rekening']	        = $row['no_rekening'];
                        $details['atas_nama_bank']      = $row['atas_nama_bank'];
                        ...
                        $details['sales_rekomend'] 	= $row['sales_rekomend'];
                    }
                    $alert = '';		
                    $returnVal = 'success';
		}else{
    ...
    function freeze_user() {
        ...
        $delete_user = $this->general_model->updateData('user_table',$data,$fieldkey);
        if($delete_user){
            $alert = 'Data profile success to '.$alert;		
            $returnVal = 'success';
        }else{
            $alert = 'Gagal menghapus , silahkan hubungi IT';		
            $returnVal = '';
    ...
    //==Add / Transfer Point to User Agent==//
    public function PointUser(){
        $post           = $this->input->post();
        $agentCode      = $post['AgentCode'];
        $MGuser_id      = substr($post['mg_user_id'],0,15);
        $agentCodeTo    = $post['AgentCodeTo'];
        $MGuser_idTo    = substr($post['mg_user_idTo'],0,15);
        $point          = $post['point'];
        $notes          = $post['notes'];
        $getAdmin       = $this->session->userdata('CMS_logged_in');
        
        $fieldKeyUser = array(
            'SUBSTRING(mg_user_id, 1, 15) = '    => $MGuser_id,
            'kode_agent'    => $agentCode,
            'status'        => 1
        );
        $fieldKeyReceiverUser = array(
            'SUBSTRING(mg_user_id, 1, 15) = '    => $MGuser_idTo,
            'kode_agent'    => $agentCodeTo,
            'status'        => 1
        );
        
        $check_availableUser = $this->admin_model->getDetData('user_table',$fieldKeyUser);
        $check_availableReceiverUser = $this->admin_model->getDetData('user_table',$fieldKeyReceiverUser);
        
        switch ($post['TipePoint']) {
            case "add":
                if($check_availableUser){
                    $addPoint   = ($check_availableUser->point + $point);
                    $updatePointUser = $this->general_model->updateData('user_table', array('point' => $addPoint), $fieldKeyUser);
                    if($updatePointUser){
                        ...
                        if($insertHistory){
                            ...
                            $alert = 'Gagal Menambahkan point , silahkan hubungi IT';		
                            $returnVal = '';
                        }
                    }else{
                        $alert = 'Gagal Menambahkan point , silahkan hubungi IT';		
                ...
                break;
            case "out":
                if($check_availableUser && $check_availableReceiverUser){
                    if($point > $check_availableUser->point){
                        $alert = 'Maaf Point Yang dikirim lebih besar dari yang dimiliki';		
                        $returnVal = '';
                    }else{
                        $PenerimaPoint  = ($check_availableReceiverUser->point + $point);
                        $PengirimPoint  = ($check_availableUser->point - $point);
                        $updatePointUserPenerima = $this->general_model->updateData('user_table', array('point' => $PenerimaPoint), $fieldKeyReceiverUser);
                        $updatePointUserPengirim = $this->general_model->updateData('user_table', array('point' => $PengirimPoint), $fieldKeyUser);
                        if($updatePointUserPenerima && $updatePointUserPengirim){
                            $dataInsertHistoryPengirim = array(
                                'user_id'           => $check_availableUser->id,
                                'status'            => 'Transfer-Point',
                                'last_point'        => $check_availableUser->point,
                                'out_point'         => $point,
                                'current_point'     => $PengirimPoint,
                                'date_create'       => date('Y-m-d H:i:s'),
                                'id_admin_approve'  => $getAdmin['id'],
                                'notes'             => $notes
                            );
                            $dataInsertHistoryPenerima = array(
                                'user_id'           => $check_availableReceiverUser->id,
                                'status'            => 'Receive-Point',
                                'last_point'        => $check_availableReceiverUser->point,
                                'in_point'          => $point,
                                'current_point'     => $PenerimaPoint,
                                'date_create'       => date('Y-m-d H:i:s'),
                                'id_admin_approve'  => $getAdmin['id'],
                                'notes'             => $notes
                            );
                            
                            $insertHistoryPengirim  = $this->general_model->insertData('history_user',$dataInsertHistoryPengirim);
                            $insertHistoryPenerima  = $this->general_model->insertData('history_user',$dataInsertHistoryPenerima);
                            if($insertHistoryPengirim && $insertHistoryPenerima){
                                $alert = 'Data point berhasil Dipindahkan';		
                                $returnVal = 'success';
                            }else{
                                $alert = 'Gagal Menambahkan point , silahkan hubungi IT';		
                                $returnVal = '';
