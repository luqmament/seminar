...
class M_report extends CI_Model {
    ...
    function report_seminar($from_date,$to_date){
        ...
        $this->db->where('DATE_FORMAT(smr.jadwal_seminar, "%Y-%m-%d") >=', $from_date);
        $this->db->where('DATE_FORMAT(smr.jadwal_seminar, "%Y-%m-%d") <=',$to_date);
        $this->db->where('ord.used_sertifikat', 1);
        $this->db->order_by('smr.jadwal_seminar','asc');
        $this->db->group_by('smr.id_seminar');
        $query = $this->db->get();
        if($query->num_rows() > 0){
